...
const EmployeeSalaryMappingPage = () => {
  const {
    mappings,
    successMessage,
    errorMessage,
  ...
  useEffect(() => {
    ...
    mappings?.forEach((mapping, index) => {
      console.log(`Mapping ${index}:`, JSON.stringify(mapping, null, 2));
    });
  }, [mappings]);
  ...
  const currentMappings = filteredMappings.slice(indexOfFirstMapping, indexOfLastMapping);
  const totalPages = Math.ceil(filteredMappings.length / mappingsPerPage);
  ...
  return (
    <div className="salary-mapping-page-container">
      ...
      {isFetching && <div className="loading-message">Loading salary mappings...</div>}
          ...
          <table className="salary-mapping-table">
            ...
            <tbody>
              {currentMappings.length > 0 ? (
                ...
                <tr>
                  <td colSpan="6">No salary mappings found</td>
                </tr>
              )}
            </tbody>
          </table>
...
export default EmployeeSalaryMappingPage;